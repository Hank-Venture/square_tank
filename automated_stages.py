...
def initStages(gpib_port):
    ...
    SD = initKlingers(cmdline, "SA", SD)
    SD = initKlingers(cmdline, "UTM", SD)
    SD = initKlingers(cmdline, "ROT", SD)

    # Initialize the non-klinger controlled MTM (aka just get its position).
    stg = "MTM"
    lib = SD[stg]["lib"]
    lib.write(cmdline, "++addr " + SD[stg]["addr"])
    SD[stg]["pos"] = lib.getPosition(cmdline, SD[stg]["axis"])
...
def initKlingers(cmd, stg, SD):
    ...
    SD[stg]["hi_step"], SD[stg]["lo_step"], SD[stg]["acc_step"] =\
                lib.setRate(cmd, stg, SD[stg]["R"], SD[stg]["S"], SD[stg]["F"])
    ...
    SD[stg]["pos"] = lib.getPosition(cmd,'0')
...
def moveStages(cmd, stage_pos, SD, ord):
    ...
    for j in range(len(stage_pos)):
        ...
        if abs(move_dist) > 0:
            ...
            SD[stg]["pos"], SD[stg]["time_st"], SD[stg]["time"] =\
                                    lib.moveRelative(cmd, stg, move_dist, SD)
...
def checkPositions(cmd, SD, ord):
    ...
    for j in range(len(ord)):
        ...
        curr_pos = lib.getPosition(cmd, axis)
        SD[stg]["pos"] = curr_pos
